...
is shaped around spending as few units as possible per qualified lead:
    ...
    videos.list            1 unit    (up to 50 videos -- durations + views)
...
class YouTubeClient:
    ...
    def videos(self, video_ids: Sequence[str]) -> list[dict[str, Any]]:
        """Duration + per-video stats for up to 50 videos per unit.

        This is what turns "channel that uploads" into "show with 74-minute
        episodes averaging 12k views" -- the runtime shape is the single most
        honest podcast signal there is, and it costs 1 unit per 50 videos.
        """
        out: list[dict[str, Any]] = []
        for chunk in _chunks([v for v in video_ids if v], 50):
            resp = self._execute(
                self.yt.videos().list(
