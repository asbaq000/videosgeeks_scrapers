...
# Podcast distribution, membership and booking hosts all get their own
# columns. Without this, every second lead's "Website" would be an
# open.spotify.com or calendly.com link instead of the show's actual site.
NON_WEBSITE_HOSTS = (
    tuple(podcast.PODCAST_HOSTS) + podcast.MEMBERSHIP_HOSTS + podcast.BOOKING_HOSTS
)


# Fields that count as "we can reach this person". A Spotify show link is
# not a way to contact anyone, so it is deliberately NOT in this list -- it
# is enrichment, not contact detail. A guest-booking form IS, because it is
# a direct channel to the host.
CONTACT_FIELDS: tuple[str, ...] = (
    "email", "instagram", "facebook", "twitter", "tiktok",
    "linkedin", "discord", "telegram", "website", "booking_link",
)
...
class Contacts(dict):
    ...
    def has_any(self) -> bool:
        return any(self.get(k) for k in CONTACT_FIELDS)
    ...
    for m in URL_RE.finditer(text or ""):
        ...
        if any(host == h or host.endswith("." + h) for h in SOCIAL_HOSTS):
            if host in ("linktr.ee", "beacons.ai", "bio.link") and url not in others:
                others.append(url)
            continue
        if any(h in url.lower() for h in NON_WEBSITE_HOSTS):
            continue
        if not website:
            website = url
...
def all_urls(text: str) -> list[str]:
    """Every http(s) URL in a blob, de-duplicated, order preserved."""
    out: list[str] = []
    for m in URL_RE.finditer(text or ""):
        url = m.group(0).rstrip(".,;:)\"'")
        if url not in out:
...
def build_contacts(
    ...
    Podcast-specific columns (Spotify / Apple / RSS / guest-booking form /
    membership) come out of the same link pool the socials do -- one pass
    over the description plus whatever the About page rendered.
    """
    ...
    links = all_urls(blob)
    platforms = podcast.detect_platforms(links)
    ...
    c = Contacts({
        ...
        # -- podcast enrichment ------------------------------------------
        "spotify": platforms.get("spotify", ""),
        "apple_podcasts": platforms.get("apple_podcasts", ""),
        "other_platform": platforms.get("other_platform", ""),
        "rss_feed": podcast.detect_rss(links),
        "booking_link": podcast.detect_booking(links),
        "membership_link": podcast.detect_membership(links),
    })
    # Every URL we saw, kept for the podcast gate -- platform links are its
    # single highest-precision signal, so it must see the raw pool, not just
    # the slots that happened to be filled above.
    c["links"] = links
    return c
