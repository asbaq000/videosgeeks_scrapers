...
def check_cadence(
    ...
    titles = [t for _, t in dated]

    if not dated:
        return CadenceResult(False, None, None, None, 0, "no uploads found", titles)

    dated.sort(key=lambda p: p[0], reverse=True)
    stamps = [d for d, _ in dated]
    ...
    if len(stamps) < min_sampled:
        return CadenceResult(
            ...
            f"only {len(stamps)} uploads -- too few to judge cadence", titles,
        )

    if days_since > max_days_since_upload:
        return CadenceResult(
            False, last_upload, round(days_since, 1), None, len(stamps),
            f"last upload {days_since:.0f}d ago > {max_days_since_upload}d", titles,
        )
    ...
    if median_gap > max_median_gap_days:
        return CadenceResult(
            ...
            f"median gap {median_gap:.0f}d > {max_median_gap_days}d", titles,
        )

    return CadenceResult(
        True, last_upload, round(days_since, 1), round(median_gap, 1), len(stamps), "", titles
    )
