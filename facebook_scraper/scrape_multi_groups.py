...
the end -- and stopping the moment the shared DAILY_LEAD_LIMIT quota (see
...
MAX_POSTS_PER_GROUP = 10  # keep each group's click-loop bounded so scanning ~30 groups is tractable
GROUP_LOAD_WAIT_SECONDS = 3
...
def looks_like_join_gate(driver) -> bool:
    ...
    try:
        containers = collector.find_post_containers(driver)
    except Exception:
        return True
    # If no article containers rendered at all after a normal page load, treat
    # it as inaccessible (private/needs-join) rather than guessing or joining.
...
def main():
    ...
    log.info("Daily lead quota: %d/%d used today.", quota_used, collector.DAILY_LEAD_LIMIT)
    if quota_used >= collector.DAILY_LEAD_LIMIT:
    ...
    # Bound each group's click-loop for this multi-group run without
    # permanently changing the main tool's default.
    collector.MAX_POSTS_PER_RUN = MAX_POSTS_PER_GROUP

    for i, (name, url) in enumerate(GROUP_URLS, start=1):
        remaining = collector.DAILY_LEAD_LIMIT - (quota_used + total_written_this_run)
        if remaining <= 0:
            log.info("Daily lead limit reached (%d/%d) -- stopping immediately before group %d/%d.",
                      quota_used + total_written_this_run, collector.DAILY_LEAD_LIMIT, i, len(GROUP_URLS))
            break
        ...
        try:
            ...
            collector.scroll_to_load_posts(driver)
            ...
            if result.collected:
                try:
                    ...
                    log.info("Wrote %d lead(s) from this group. Daily quota now %d/%d.",
                              written, quota_used + total_written_this_run, collector.DAILY_LEAD_LIMIT)
    ...
    log.info("Daily multi-group scrape finished. New leads saved this run: %d. "
              ...
              total_written_this_run, quota_used + total_written_this_run, collector.DAILY_LEAD_LIMIT,
